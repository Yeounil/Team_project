...
class RoundRobin:
    ...
    def schedule(self, ready_queue, pcores, ecores):
        time       = 0  # 전체 시뮬레이션 시간
        all_cores  = pcores + ecores # 모든 코어 목록화
  ...
  # ── 3) 메인 스케줄링 루프 ── , 모든 프로세스가 종료할 때 까지 아래 4가지의 로직을 반복
            ...
            for i, core in enumerate(all_cores):
              ...
              # (1) 프로세스가 완전 종료된 경우(타임 퀀텀에 의해 벗어난 것이 아닌, 프로세스 실행 시간이 다 된 경우: 종료 처리 및 코어 비우기)
                ...
                if running[i] is None and rr_queue:
                    for p in rr_queue: # 아직 실행이 끝나지 않았고, 이번 tick에 이미 할당되지 않은 프로세스만
        ...
        for p in ready_queue:
            ...
            p.turn_around_time = p.finish_time - p.arrival_time  # 반환 시간: 전체 걸린 시간
            p.waiting_time     = p.turn_around_time - p.real_burst # 대기 시간 = 반환 - 실제 서비스 시간
            p.normalized_TT    = round(p.turn_around_time / p.real_burst, 2) # 정규화 TT
